...
divhah_compress (DivideHah width consonants vowels) str =
  map (\(ch, _) -> ch) $ List.sortBy (Ord.comparing snd) $ divcv $ zip str [0..]
  where divcv tlist = let clist = filter (\(str, _) -> elem str consonants) tlist
                          vlist = filter (\(str, _) -> elem str vowels) tlist
