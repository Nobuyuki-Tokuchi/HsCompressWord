...
getOption (op@"-c":val@(h:_):xs) option = case h of
  ...
  _ -> getOption xs option{ count = read val }
-- mode option
...
getOption ("--separate":xs) option = getOption xs option { mode = "separate" }
getOption ("-s":xs) option = getOption xs option { mode = "separate" }
...
getOption (op@"--consonants":val@(h:_):xs) option = case h of
  '-' -> Left ("argument to '" ++ op ++ "' is missing")
  _ -> getOption xs option { consonants = val }
getOption [op@"-C"] _ = Left ("argument to '" ++ op ++ "' is missing")
getOption (op@"-C":val@(h:_):xs) option = case h of
  '-' -> Left ("argument to '"++ op ++"' is missing")
  _ -> getOption xs option { consonants = val }
...
getOption (op@"--vowels":val@(h:_):xs) option = case h of
  '-' -> Left ("argument to '"++ op ++"' is missing")
  _ -> getOption xs option { vowels = val }
...
help = putStrLn $ unlines [
  ...
  "  -s, --separate : SeparateHah Compress.",
...
run (Right Version) = putStrLn "0.3.0"
run (Right Help) = help
run (Right Parameters { count = c, word = w, mode = m, consonants = cs, vowels = vs }) = putStrLn $ WComp.compress compMode w where
  compMode = case m of
    ...
    "separate" -> WComp.SeparateHah c cs vs
